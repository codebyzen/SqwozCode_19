...
class mrs:
 ...
 453: 'Японский язык'}
	...
	# в скрипт прилетает название активности, а запросом к базе мы должны забрать ml_id из таблицы groups_PROD,
	# соответствующий реальному названию активности
	...
	def count_online_top10_Z(self, recs):
		return sum([int('ОНЛАЙН' in r[1]) for r in recs[:10]])


	def del_online_from_top10(self, recs):
		while (self.count_online_top10(recs) > 7 and len(recs) > 10):
			for k in range(9, 0, -1):
				if 'ОНЛАЙН' in recs[k][1]:
					del recs[k]
					break
		return recs[:10]

	def del_online_from_top10_Z(self, recs):
		while (self.count_online_top10(recs) > 7 and len(recs) > 10):
			for k in range(9, 0, -1):
				if 'ОНЛАЙН' in recs[k]:
					del recs[k]
	...
	def init(self, groupid):
		
		config = config_object()
		sqlEngine = create_engine(config.mysql_str(), pool_recycle=3600)
		dbConnection = sqlEngine.connect()
		
		self.df_groups = pd.read_sql("select * from `ml_groups` WHERE groupid = "+str(groupid), dbConnection)
		item_id = self.df_groups['ml_id'][0]
		...
		similar, _ = model.similar_items(item_id, N = 31)
		# print(similar)

		

		# print(sum(['ОНЛАЙН' in self.d3_reverse_mapping[s] for s in similar]))
		similar_items = [self.d3_reverse_mapping[i] for i in similar][1:]

		similar_array = [list(x) for x in zip(similar[1:],similar_items)]
		# print(test)
		
		# print('Request:', self.d3_reverse_mapping[item_id])
		# print('Similar (with online filtration):')
		# print(self.del_online_from_top10(similar_items))
		
		# result = self.del_online_from_top10(similar_items)
		result = self.del_online_from_top10_Z(similar_array)


		last_final_newFolder999_copy_result = []
		for k, i in enumerate(result):
			group = pd.read_sql("select * from `ml_groups` WHERE ml_id = "+str(result[k][0]), dbConnection)
			last_final_newFolder999_copy_result.append(int(group['groupid'][0]))
			# result[k][0] = str(result[k][0]) # for future JSON.dump int32->int

		dbConnection.close()
		return last_final_newFolder999_copy_result